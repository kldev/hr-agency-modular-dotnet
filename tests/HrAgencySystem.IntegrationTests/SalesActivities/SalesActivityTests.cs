...
public sealed class SalesActivityTests(IntegrationEnvironment env, ITestOutputHelper output)
    : BaseIntegrationTest(env, output)
{
    ...
    {
        await SalesActivityTestClient.Create(organizationId: _organizationId);

        await SalesActivityTestClient.Create(organizationId: _organizationId);

        await SalesActivityTestClient.Create(organizationId: Guid.NewGuid());

        await Eventually.AssertAsync(async () =>
        {
            var slice = await SalesActivityTestClient.GetSliceAsync(
                organizationId: _organizationId
            );
    ...
    public async Task ShouldNotGetOtherOrganizationActivities()
    {
        ...
        await SalesActivityTestClient.Create(organizationId: otherOrganization);
