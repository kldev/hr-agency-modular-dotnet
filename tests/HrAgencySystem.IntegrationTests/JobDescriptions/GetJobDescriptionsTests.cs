...
) : BaseIntegrationTest(environment, output)
{
    ...
    private async Task SetupData()
    {
        ...
        await JobDescriptionClient.CreateAsync(
            JobDescriptionTestData.CreateRequest(companyId: CompanyId, recruiterId: RecruiterId)
        );
    ...
    private async Task ShouldGetJobDescriptionsFromOrganization()
    {
        await Eventually.AssertAsync(async () =>
        {
            ...
            Assert.DoesNotContain(result, x => x.Title == "Other Organization Developer");
        });
    }
    ...
    private async Task ShouldFilterByCompany()
    {
        await Eventually.AssertAsync(async () =>
        {
            var result = await GetJobDescriptionsAsync(companyId: CompanyId);
    ...
    private async Task ShouldFilterByRecruiter()
    {
        await Eventually.AssertAsync(async () =>
        {
            var result = await GetJobDescriptionsAsync(recruiterId: RecruiterId);
            Assert.Equal(2, result.Count);

            Assert.All(result, x => Assert.Equal(RecruiterId, x.RecruiterId));
        });
    }
    ...
    private async Task ShouldFilterByStatus()
    {
        await Eventually.AssertAsync(async () =>
        {
            var result = await GetJobDescriptionsAsync(status: [JobDescriptionStatus.Draft]);

            Assert.All(result, x => Assert.Equal(JobDescriptionStatus.Draft, x.Status));
        });
    }
    ...
    private async Task ShouldFilterByMultipleStatuses()
    {
        ...
        {
            var result = await GetJobDescriptionsAsync(
                status: [JobDescriptionStatus.Draft, JobDescriptionStatus.Open]
            );
            ...
            Assert.All(
                ...
                x =>
                    Assert.Contains(
                        x.Status,
                        new[] { JobDescriptionStatus.Draft, JobDescriptionStatus.Open }
                    )
            );
        });
    }
    ...
    private async Task ShouldPaginate()
    {
        ...
        await Eventually.AssertAsync(async () =>
        {
            firstPage = await GetJobDescriptionsAsync(page: 1, pageSize: 2);
        ...
        await Eventually.AssertAsync(async () =>
        {
            var secondPage = await GetJobDescriptionsAsync(page: 2, pageSize: 2);
    ...
    private async Task ShouldNotGetJobDescriptionsFromOtherOrganization()
    {
        ...
        await Eventually.AssertAsync(async () =>
        {
            var result = await GetJobDescriptionsAsync();
            Assert.Single(result);
            Assert.Equal("Other Organization Developer", result[0].Title);
        });
    ...
    {
        var statusUrl =
            status != null & status is { Length: > 0 }
                ? "&status=" + string.Join("&status=", status!)
                : "";

        var url =
            $"/api/job-description?search={search}&companyId={companyId ?? Guid.Empty}&recruiterId={recruiterId ?? Guid.Empty}{statusUrl}";
        ...
        OutputHelper.WriteLine($"Returned {result.Content.Count} job descriptions.");
