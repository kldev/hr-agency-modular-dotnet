...
public class CandidateGetSliceTests(IntegrationEnvironment environment, ITestOutputHelper output)
    : BaseIntegrationTest(environment, output)
{
    ...
    public async Task ShouldGetCandidateSlice()
    {
        ...
        await Eventually.AssertAsync(async () =>
        {
            ...
            var candidate = Assert.Single(result.Content, x => x.Id == created.CandidateId);
    ...
    public async Task ShouldReturnAllCandidates()
    {
        ...
        await Eventually.AssertAsync(async () =>
        {
            ...
            Assert.Contains(result.Content, x => x.Id == first.CandidateId);

            Assert.Contains(result.Content, x => x.Id == second.CandidateId);

            Assert.Contains(result.Content, x => x.Id == third.CandidateId);
        });
    }
    ...
    public async Task ShouldRespectPageSize()
    {
        ...
        await Eventually.AssertAsync(async () =>
        {
            var result = await CandidateClient.GetSliceAsync(_organizationId, page: 1, pageSize: 2);
    ...
    public async Task ShouldReturnNextSlice()
    {
        ...
        await Eventually.AssertAsync(async () =>
        {
            ...
            Assert.Equal(first.CandidateId, secondPageCandidate.Id);
