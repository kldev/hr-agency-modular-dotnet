...
public class CreateJobPostHandlerTests : BaseTest
{
    ...
    private static UserSnapshot Recruiter { get; } =
        new(RecruiterId, "Alice", "Wells", "alice-wells@hr-agency.com");
    ...
    public async Task Handle_WithValidCommand_ReturnsJobPostCreated()
    {
        ...
        var jobDescription = new JobDescriptionSnapshot(jobDescriptionId, "Test", companyId);

        var company = new CompanySnapshot(companyId, "Company A", "TX-100-101");

        _service
            .GetOrganizationSlug(OrganizationId.From(organizationId), Arg.Any<CancellationToken>())
            .Returns("company-a");
        ...
        _jobDescriptionSnapshotRepository
            .GetAsync(jobDescriptionId, organizationId, Arg.Any<CancellationToken>())
            .Returns(jobDescription);

        _service.GetCompanyAsync(companyId, Arg.Any<CancellationToken>()).Returns(company);

        var result = await Handle(command, new FixedClock(now));
        ...
        Assert.Equal("PL", result.LanguageCode);
        ...
        Assert.Equal("company-a", result.OrgSlug);
        ...
        await _service.Received(1).GetUserAsync(RecruiterId, Arg.Any<CancellationToken>());
        ...
        _documentSession
            ...
            .StartStream<JobPost>(
                result.JobPostId,
                Arg.Is<JobPostCreated>(x => x.JobPostId == result.JobPostId)
            );
    }
    ...
    public async Task Handle_WithNonExistingRecruiter_ThrowsBusinessRuleException()
    {
        ...
        var exception = await Assert.ThrowsAsync<NotFoundException>(() =>
            Handle(CreateValidCommand(recruiterId: recruiterId))
        );
    ...
    public async Task Handle_WithNonExistingCreatedByUser_ThrowsBusinessRuleException()
    {
        ...
        var exception = await Assert.ThrowsAsync<NotFoundException>(() =>
            Handle(CreateValidCommand(createdBy: createdById))
        );
    ...
    public async Task Handle_WithNonExistingJobDescription_ThrowsBusinessRuleException()
    {
        ...
        Assert.Equal(IJobDescriptionSnapshotRepository.NotFoundMessage, exception.Message);
    ...
    public async Task Handle_WithNonExistingCompany_ThrowsBusinessRuleException()
    {
        ...
        _jobDescriptionSnapshotRepository
            .GetAsync(jobDescriptionId, organizationId, Arg.Any<CancellationToken>())
            .Returns(new JobDescriptionSnapshot(jobDescriptionId, "TEST", companyId));
    ...
    private async Task<JobPostCreated> Handle(CreateJobPost command, IClock? clock = null)
    {
    ...
    private void SetupOrganization(string slug = "company-a")
    {
        _service
            .GetOrganizationSlug(Arg.Any<OrganizationId>(), Arg.Any<CancellationToken>())
            .Returns(slug);
    }
    ...
    private void AssertNoStream()
    {
        _documentSession
            ...
            .StartStream<JobPost>(Arg.Any<Guid>(), Arg.Any<object>());
    }
