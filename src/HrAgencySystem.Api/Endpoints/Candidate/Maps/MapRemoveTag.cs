...
internal static class MapRemoveTag
{
    internal static void Map(RouteGroupBuilder group)
    {
        ...
        group
            .MapDelete(ApiEndpoints.Recruitment.Candidates.RemoveTag, Handler)
            .WithSummary("Remove tag")
            .WithName("Remove candidate tag")
    ...
    {
        var result = await bus.InvokeAsync<CandidateTagRemoved>(
            new RemoveCandidateTag(candidateId, tagId, user.OrganizationId, user.UserId),
            ct
        );
