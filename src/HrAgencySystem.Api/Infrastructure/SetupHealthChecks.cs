...
public static class SetupHealthChecks
{
    ...
    {
        /// <summary>
        /// What <c>/health/ready</c> asks. The file and reports services are degraded rather than
        /// unhealthy when they are gone: the API still serves everything except documents and reports,
        /// and an orchestrator should not pull a whole API node out of rotation for that.
        /// </summary>
        public void AddApiHealthChecks(IConfiguration configuration)
        {
            services
                ...
                .AddCheck<ReportsHealthCheck>("reports-service", tags: HealthTags.ReadyOnly);
        }
    }
}
