...
public sealed class OrganizationChecker(IQuerySession session) : IOrganizationChecker
{
    public async Task<bool> Exists(Guid organizationId, CancellationToken ct)
    {
    ...
    public async Task<OrganizationId> GetOrganizationIdBySlug(string slug, CancellationToken ct)
    {
        ...
        if (organizationId == Guid.Empty)
            throw new NotFoundException("Organization", "Slug");
