...
internal static class SalesProjectionConfiguration
{
    ...
    private static void ConfigureActivityProjection(StoreOptions options)
    {
        options.Projections.Snapshot<ActivityProjection>(SnapshotLifecycle.Async);

        options
            .Schema.For<ActivityProjection>()
    ...
    private static void ConfigureOpportunityProjection(StoreOptions options)
    {
        options.Projections.Snapshot<OpportunityProjection>(SnapshotLifecycle.Async);

        options
            .Schema.For<OpportunityProjection>()
            .DatabaseSchemaName(SchemaName)
            .Index(x => new { x.OrganizationId })
            .Index(x => new { x.OrganizationId, ca = x.CreatedAt })
            .Index(
                x => new { x.OrganizationId, c = x.CompanyId },
                idx =>
                {
                    idx.Name = "idx_sop_company";
                }
            )
            .Index(
                x => new { x.OrganizationId, s = x.Stage },
                idx =>
                {
                    idx.Name = "idx_sop_stage";
                }
            )
            .Index(
                x => new { x.OrganizationId, o = x.ResponsibleId },
                idx =>
                {
                    idx.Name = "idx_sop_owner_id";
                }
            )
            .Index(
                x => new
    ...
    private static void ConfigureSalesPipelineStageSummaryProjection(StoreOptions options)
    {
        ...
        options
            .Schema.For<PipelineStageSummary>()
            .DatabaseSchemaName(SchemaName)
            .Index(x => new { x.OrgId })
