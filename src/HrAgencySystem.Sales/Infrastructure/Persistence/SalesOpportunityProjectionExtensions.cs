...
internal static class SalesOpportunityProjectionExtensions
{
    ...
    {
        return responsibleId.IsInvalid()
            ? query
            : query.Where(p => p.ResponsibleId == responsibleId);
    }
    ...
    )
    {
        return string.IsNullOrEmpty(search)
            ? query
            : query.Where(p =>
                p.Company.Name.Contains(search, StringComparison.OrdinalIgnoreCase)
    ...
    internal static IQueryable<OpportunityProjection> WithResponsibleIds(
        this IQueryable<OpportunityProjection> query,
        IReadOnlyList<Guid> responsibleIds
    )
    {
        return query.Where(p => responsibleIds.Contains(p.ResponsibleId));
    }
}
