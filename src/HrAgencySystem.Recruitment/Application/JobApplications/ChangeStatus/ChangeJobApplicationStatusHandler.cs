...
public static class ChangeJobApplicationStatusHandler
{
    ...
    )
    {
        ...
        var @event = new JobApplicationStatusChanged(
            aggregate.Id.Value,
            aggregate.CandidateId.Value,
            now,
            oldStatus,
            newStatus,
            user
        ...
        var events = new List<IJobApplicationEvent> { concreteEvent, @event };

        if (string.IsNullOrEmpty(command.Note))
            return (result, [.. events]);

        var noteAddedEvent = await CreateApplicationNoteAddedEvent(
            command,
            aggregate,
            noteRepository,
            user,
    ...
    private static void ValidatePolicy(JobApplication aggregate, JobApplicationStatus newStatus)
    {
        ...
        if (!changeAllowed)
            throw new BusinessRuleException(
                $"Not allowed to change job application status form {aggregate.Status} to {newStatus}"
            );
    }

    private static async Task<JobApplicationNoteAdded> CreateApplicationNoteAddedEvent(
        ChangeJobApplicationStatus command,
        JobApplication aggregate,
        INoteRepository noteRepository,
        UserSnapshot user,
    ...
    )
    {
        ...
        var saveNote = new CreateNoteDocument(
        ...
        var noteAddedEvent = new JobApplicationNoteAdded(
            aggregate.Id.Value,
            aggregate.CandidateId.Value,
            now,
            command.Note,
    ...
    private static IJobApplicationEvent GetConcreteEvent(
        ChangeJobApplicationStatus command,
        DateTimeOffset now,
        UserSnapshot user
    )
    {
        ...
        {
            JobApplicationStatus.Screening => new JobApplicationScreeningStarted(
                command.JobApplicationId,
                now,
                user
            ),
            JobApplicationStatus.Interview => new JobApplicationInterviewScheduled(
                command.JobApplicationId,
                now,
                user,
                command.InterviewId!.Value
            ),
            JobApplicationStatus.Assessment => new JobApplicationAssessmentStarted(
                command.JobApplicationId,
                now,
                user
            ),
            JobApplicationStatus.Offer => new JobApplicationOfferMade(
                command.JobApplicationId,
                now,
                user
            ),
            JobApplicationStatus.Rejected => new JobApplicationRejected(
                command.JobApplicationId,
                now,
                user
            ),
            JobApplicationStatus.Withdrawn => new JobApplicationWithdrawn(
                command.JobApplicationId,
                now,
                user
            ),
            JobApplicationStatus.Hired => new JobApplicationHired(
                command.JobApplicationId,
                now,
                user
            ),
...
public record ChangeJobApplicationStatusResult(
    JobApplicationStatus OldStatus,
    JobApplicationUpdateStatus NewStatus
);
