...
{
    public async Task<JobPostActiveChangedIntegrationEvent?> HandleAsync(
        JobPostStatusChanged message,
        ILogger<JobPostStatusChanged> logger
    )
    {
        ...
        return delta == 0
            ? null
            : new JobPostActiveChangedIntegrationEvent(message.JobPostId, message.CompanyId, delta);
    }
}
