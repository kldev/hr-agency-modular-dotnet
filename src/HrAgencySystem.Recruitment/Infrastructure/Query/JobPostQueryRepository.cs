...
{
    public async Task<SliceResponse<JobPostResponse>> GetJobPosts(
        Guid organizationId,
        JobPostQuery query,
    ...
    )
    {
        ...
        var response = result.Content.Select(JobPostResponse.From).ToList();
    ...
    public async Task<JobPostProjection?> GetJobPost(
        Guid organizationId,
        Guid jobPostId,
        CancellationToken ct
    )
