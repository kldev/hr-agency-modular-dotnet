...
{
    internal static (CompanyData, List<string> errors) CreateCompanyData(
        ICompanyData data,
    ...
    )
    {
        ...
        var (registrationNumber, registrationNumberError) = RegistrationNumber.TryCreate(
            data.RegistrationNumber
        );
        var (countryCode, countryError) = CountryCode.TryCreate(data.CountryCode);
        var (webSite, webSiteError) = WebSite.TryCreate(data.WebSite);
