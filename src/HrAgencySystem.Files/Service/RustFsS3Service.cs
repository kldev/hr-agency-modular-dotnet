...
public class RustFsS3Service : IFileStorage
{
    ...
    private async Task<FileResponse> GetObjectStreamAsync(
        string key,
        CancellationToken token,
        string useBucketName = ""
    )
    {
    ...
    public async Task EnsureBucketExistsAsync(CancellationToken token, string bucketName = "")
    {
        try
        {
    ...
    {
        var request = new PutBucketRequest { BucketName = useBucketName };
