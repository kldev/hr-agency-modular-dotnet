import type { ComplianceRequirement, ComplianceStatus, EngagementType } from "@/api/models";

/*
 * The vocabulary of HrAgencySystem.Compliance, which is a shared domain library on the backend for
 * the same reason this file is shared here: projects and assignments answer the same catalogue at
 * two different levels, and five of the requirements moved from one to the other when the worker
 * register arrived. One copy of the words, or the two screens start disagreeing about the law.
 */
...
export const complianceRequirementHints: Record<ComplianceRequirement, string> = {
	A1Certificates:
		"Issued to one named person for one period by the country whose social security they stay in. Recorded on the assignment, because that is how it is issued.",
	...
	DeLongTermPostingNotification:
		"Filed before the 12th month ends, it extends this person's posting to 18 months. After that, full German employment conditions apply.",
...
/**
 * Mirrors `ComplianceCatalogue.PerPerson` on the backend: requirements issued to one named person
 * for one period rather than held by the delivering entity for everybody.
 *
 * Both catalogue endpoints already filter by scope, so nothing depends on this list to decide what
 * to show. It is here so a screen can explain *where* a requirement belongs — which is the question
 * the backend answers with a refusal, and a refusal is a poor first explanation.
 */
export const assignmentScopedRequirements: ComplianceRequirement[] = [
	"A1Certificates",
	"BeLimosaDeclaration",
	"BeSocialDocumentsExemption",
	"BeMotivatedNotification",
